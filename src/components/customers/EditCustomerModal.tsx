...
interface EditCustomerModalProps {
  ...
  onClose: () => void;
  onSave: (custno: string, data: Partial<Pick<Customer, 'custname' | 'address' | 'payterm'>>) => Promise<void>;
  loading: boolean;
  error: string | null;
  C: DashboardTokens;
  isDark: boolean;
...
export const EditCustomerModal: React.FC<EditCustomerModalProps> = ({
  isOpen, customer, onClose, onSave, loading, error, C, isDark,
}) => {
  ...
  useEffect(() => {
    if (isOpen && customer) {
      ...
      setPayterm(customer.payterm || 'COD');
    }
  }, [isOpen, customer]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    void onSave(customer.custno, { custname, address, payterm });
  };
